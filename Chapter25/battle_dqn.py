...
def test_model(net: model.DQNModel, device: torch.device, gw_config) -> Tuple[float, float, float, float]:
    ...
    return a_rewards / COUNT_AGENTS_1, b_steps / COUNT_AGENTS_1, b_rewards / COUNT_AGENTS_2, b_steps / COUNT_AGENTS_2
...
if __name__ == "__main__":
    ...
    net = model.DQNModel(obs.spaces[0].shape, obs.spaces[1].shape, m_env.get_action_space(a_handle)[0]).to(device)
    ...
    a_exp_source = ptan.experience.ExperienceSourceFirstLast(a_env, agent, PARAMS.gamma, vectorized=True)
    ...
    engine.run(batch_generator(a_exp_source, b_exp_source, buffer, PARAMS.replay_initial, PARAMS.batch_size))
