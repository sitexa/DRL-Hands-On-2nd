...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", required=True, help="Category to use for training. Empty " "string to train on full dataset"
    )
    ...
    parser.add_argument(
        "--disable-skip", default=False, action="store_true", help="Disable skipping of samples with high argmax BLEU"
    )
    ...
    net = model.PhraseModel(emb_size=model.EMBEDDING_DIM, dict_size=len(emb_dict), hid_size=model.HIDDEN_STATE_SIZE).to(
        device
    )
    ...
    with ptan.common.utils.TBMeanTracker(writer, 100) as tb_tracker:
        ...
        for epoch in range(MAX_EPOCHES):
            ...
            if epoch % 10 == 0:
                torch.save(
                    net.state_dict(), os.path.join(saves_path, "epoch_%03d_%.3f_%.3f.dat" % (epoch, bleu, bleu_test))
                )
