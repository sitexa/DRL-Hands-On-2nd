...
class Net(nn.Module):
    def __init__(self, input_shape, actions_count):
        ...
        self.body = nn.Sequential(nn.Linear(self.input_size, 4096), nn.ELU(), nn.Linear(4096, 2048), nn.ELU())
...
def make_train_data(
    cube_env, net, device, batch_size, scramble_depth, shuffle=False, value_targets=ValueTargetsMethod.Paper
):
    assert isinstance(cube_env, cubes.CubeEnv)
