...
class DQNModel(nn.Module):
    def __init__(self, obs_size: int, cmd_size: int, hid_size: int = 256):
        ...
        self.net = nn.Sequential(nn.Linear(obs_size + cmd_size, hid_size), nn.ReLU(), nn.Linear(hid_size, 1))
...
class DQNAgent(ptan.agent.BaseAgent):
    def __init__(self, net: DQNModel, preprocessor: preproc.Preprocessor, epsilon: float = 0.0, device="cpu"):
        self.net = net
...
@torch.no_grad()
def unpack_batch(
    batch: List[ptan.experience.ExperienceFirstLast], preprocessor: preproc.Preprocessor, net: DQNModel, device="cpu"
):
    """
...
class CommandModel(nn.Module):
    def __init__(
        ...
        self.rnn = nn.LSTM(input_size=embeddings.embedding_dim, hidden_size=obs_size, batch_first=True)
...
class CmdAgent(ptan.agent.BaseAgent):
    ...
    def __call__(self, states, agent_states=None):
        ...
        for state in states:
            ...
            tokens = [self.env.action_space.id2w[t] for t in cmd if t not in {self.cmd.sep_token, self.cmd.start_token}]
            action = " ".join(tokens)
...
def unpack_batch_dqncmd(
    batch, prep: preproc.Preprocessor, cmd: CommandModel, cmd_encoder: preproc.Encoder, net: DQNModel, env: gym.Env
):
...
def calc_loss_dqncmd(batch, preprocessor, cmd, cmd_encoder, tgt_cmd_encoder, net, tgt_net, gamma, env, device="cpu"):
